...
BIND_PORT_TCP=${BIND_ADDRESS:-7000}
BIND_PORT_UDP=${BIND_ADDRESS:-}
...
APPEND_CONFIG_FILE=${APPEND_CONFIG_FILE:-}
# -------------------------------------------------------
...
if [[ -n "${APPEND_CONFIG_FILE}" ]]; then
    if [[ -f "${APPEND_CONFIG_FILE}" ]]; then
        echo "# Appending config file..." >> ${config_filename}
        cat ${APPEND_CONFIG_FILE} >> ${config_filename}
    else
        printf "<${APPEND_CONFIG_FILE}> is missing. Skipped it." >&2
    fi
fi
